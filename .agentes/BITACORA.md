...
[2026-09-04 11:40] [CLAUDE] G-009 el asistente contesta bien pero tardaba 12-13s: gemini-3.5-flash-lite esta saturado (503) y el worker lo intentaba primero, perdiendo segundos antes de pasar al que si responde (3.1-flash-lite). Reordenados los modelos y agregado un corte de 9s por intento, para que un modelo caido no le sume su demora a todas las respuestas. Verificado el comportamiento: da el plazo real del tercer eje, se niega a dar precios aun insistiendo, no inventa toneladas ni plazos que no sabemos, rechaza un intento de saltarse las reglas y no opina de politica | asistente/worker.js

[2026-09-04 11:45] [CLAUDE] G-009 revertido el precio: Leandro decidio no publicarlo, para no perder al cliente que se autodescarta antes de escribir. El worker vuelve a mandar todas las consultas de precio al presupuesto. La implementacion queda en el historial (commit 6aea462) por si cambia de opinion | asistente/worker.js

[2026-09-04 11:47] [CLAUDE] G-009 respuestas cortadas a la mitad: maxOutputTokens estaba en 500, que alcanzaba para los modelos 2.5 pero no para los 3.x, que gastan parte de ese presupuesto pensando antes de escribir. Subido a 1200. Ademas el corte por intento bajo a 8s y el diagnostico ahora informa cuanto tardo cada modelo y si la respuesta quedo truncada (finishReason) | asistente/worker.js

[2026-09-04 11:50] [CLAUDE] G-009 el corte por intento (8s) mataba respuestas buenas: una pregunta que da para pensar tarda mas de 8s y moria degollada en los cuatro modelos, dejando al visitante sin nada. Cambiado por un limite del TOTAL de la espera (20s) que se reparte entre los intentos, y los dos modelos que responden quedaron primeros. Diagnostico: 3.1-flash-lite 1,8s y 3.6-flash 2,7s sanos; 3.5-flash-lite y flash-lite-latest siguen dando 503 | asistente/worker.js

[2026-09-04 11:55] [CLAUDE] G-009 PUBLICADO. Asistente andando en las 11 paginas y verificado en vivo: contesta con datos reales en 2-3s, no da precios ni aun insistiendo, no inventa plazos ni toneladas, rechaza un intento de saltarse las reglas, dice que no hacen mecanica de autos, y CORS deja pasar solo los 3 origenes del sitio. Ninguna clave publicada. Ultimo retoque pendiente de pegar: que no vuelva a pedir la unidad si ya se la dieron | assets/js/main.js, assets/css/styles.css, asistente/worker.js, CLAUDE.md

[2026-09-04 13:48] [CLAUDE] G-010 las 11 descripciones estaban entre 171 y 271 caracteres y Google corta en ~155: la de la portada perdia las certificaciones y la ciudad, que es lo que convierte. Reescritas a 133-149, sin dato nuevo, solo recortando y reordenando lo que ya decia cada pagina. Actualizadas tambien las og:description y el lastmod del sitemap | los 11 index.html, sitemap.xml

[2026-09-04 14:07] [CLAUDE] Corrido /impeccable init: escrito PRODUCT.md con el contexto de producto (usuarios confirmados por Leandro: dueno de un camion y responsable de flota; posicionamiento: la cadena fabricacion propia -> homologacion -> trabajos que otros rechazan). Queda abierta una decision: cual de las cuatro encabeza cuando hay lugar para una sola frase. G-011: asistente/ y versionar.ps1 se estaban sirviendo desde el dominio (200); sumados a la lista de exclusion junto con PRODUCT.md | PRODUCT.md, _config.yml

[2026-09-04 14:26] [CLAUDE] Corrida /impeccable critique sobre la portada (dos agentes aislados): 25/40, "Aceptable". Instantanea en .impeccable/critique/. G-012 arreglados los tres defectos que habia introducido G-007 y que encontro la critica: el desplegable arrancaba en "Colocacion de 3er eje" y mandaba trabajos falsos, el enlace del pie no mostraba el selector, y window.open no se verificaba (conversion contada sin que WhatsApp abriera). Verificados los cuatro caminos en el navegador. Ojo: el agente de diseno afirmo que el mapa dice "Bv San Diego" y es FALSO, las 46 menciones dicen "Av" | assets/js/main.js, assets/css/styles.css, PRODUCT.md, los 11 index.html
